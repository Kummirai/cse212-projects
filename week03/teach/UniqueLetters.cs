public static class UniqueLetters
{
    ...
    private static bool AreUniqueLetters(string text)
    {
        ...
        //2. Check if item exists in the set, if not we add element to the set

        //3. if element exists we return 
        for (var i = 0; i < text.Length; i++)
        {
            if (newSet.Contains(text[i].ToString()))
            {
                return false;
            }
            else
            {
                newSet.Add(text[i].ToString());
            }
        }
